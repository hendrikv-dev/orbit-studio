/**
 * The visual evidence for the map-first Tracker pass.
 *
 * ## What this is for
 *
 * A review package is only worth reading if its pictures are of the states they
 * claim to be of. So each shot here is taken by driving the real product to the
 * real state and then *checking* that the state arrived — a shot whose
 * precondition failed is recorded as a failure rather than saved as a plausible
 * looking picture of something else.
 *
 * ## Live data and fixtures
 *
 * The basemap is live: the point of these pictures is to show the product as a
 * reader sees it, and an empty style would show a map with no map in it.
 *
 * Three feeds are fixtured, and every caption says so. Orbits and the cloud
 * mask are pinned because the states the brief asks for — a visible station
 * pass, an observed cloud field — are states the sky has to cooperate with, and
 * a review package that can only be produced on a clear night with the ISS
 * overhead is a review package nobody can reproduce. The fixtures are the same
 * ones the refinement gate asserts against, imported rather than copied, so the
 * picture and the assertion are about the same night.
 *
 * Nothing here fixtures the astronomy. Positions, rise and set times, twilight
 * and the ranking are computed from the real ephemeris at the pinned instant.
 */
...
const OUT = path.join(process.cwd(), "review", "tracker-pass");
 ...
 */
async function capture(page, id, caption, expect, options = {}) {
  let found = null;
  try {
    found = await expect();
  } catch (error) {
  ...
  const file = `${id}.png`;
  await page.screenshot({ path: path.join(SHOTS, file), ...options });
...
async function main() {
  /**
   * Clear only what this script produces.
   *
   * It used to remove the package directory whole, which also deleted the
   * hand-written documents sitting beside the screenshots — SUMMARY, GATES,
   * LIMITATIONS. Regenerating evidence should never take the prose that
   * explains it with it.
   */
  await rm(SHOTS, { recursive: true, force: true });
  await rm(path.join(OUT, "contact-sheet.png"), { force: true });
  ...
  /**
   * The contact sheet, rendered rather than composited.
   *
   * Built as a page and photographed, so it carries each shot's caption and
   * whether its precondition was actually met — which is the part of a contact
   * sheet that does any work. Doing it this way also keeps the script free of a
   * system image binary: Playwright is already here, ImageMagick may not be.
   */
  await writeFile(path.join(SHOTS, "contact-sheet.html"), contactSheetHtml(shots));
  {
    const context = await browser.newContext({
    ...
    const page = await context.newPage();
    await page.goto(`file://${path.join(SHOTS, "contact-sheet.html")}`, {
      waitUntil: "load",
    });
    await page.waitForTimeout(1500);
  ...
  await writeFile(
    path.join(OUT, "screenshots", "manifest.json"),
    `${JSON.stringify({ capturedUtc: new Date().toISOString(), pinnedClockUtc: SATELLITE_CLOCK.toISOString(), shots }, null, 2)}\n`,
  );
...
function contactSheetHtml(entries) {
  const cards = entries
    .map(
      (shot) => `
      <figure>
  ...
  return `<!doctype html>
<meta charset="utf-8">
<title>Tracker pass — contact sheet</title>
<style>
...
<h1>Tracker map-first pass — contact sheet</h1>
<p class="lede">
